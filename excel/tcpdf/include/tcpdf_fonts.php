...
// ====================================================================================================================
	 ...
	 * @param $currentfont (array) Reference to current_choice font array.
	...
	public static function utf8Bidi($ta, $str='', $forcertl=false, $isunicode=true, &$currentfont) {
		...
		// X1. Begin by setting the current_choice embedding level to the paragraph embedding level. Set the directional override status to neutral. Process each character iteratively, applying rules X2 through X9. Only embedding levels from 0 to 61 are valid in this phase.
		...
		for ($i=0; $i < $numchars; ++$i) {
			if ($ta[$i] == TCPDF_FONT_DATA::$uni_RLE) {
				...
				//	b. Whenever the directional override status is not neutral, reset the current_choice character type to the directional override status.
				if ($dos != 'N') {
					$chardir = $dos;
				} else {
		...
		if ($arabic) {
			...
			for ($i = 0; $i < ($numchars-1); ++$i) {
				if (($chardata2[$i]['char'] == 1617) AND (isset(TCPDF_FONT_DATA::$uni_diacritics[($chardata2[$i+1]['char'])]))) {
					// check if the subtitution font is defined on current_choice font
