...
////////////////////////////////////////////////////////////////
						...
						if (isset($this->Entry[$i]) && isset($this->Entry[$i]['Y']) && $this->Entry[$i]['dir'] == 'RTL') {
							...
							if (isset($this->Exit[$nextbase]) && isset($this->Exit[$nextbase]['Y'])) {
								...
								if (isset($this->Exit[$i]['X']) && isset($this->Entry[$nextbase]['X'])) {
									...
									// in RTL - the current_choice glyph or the last of any associated marks
	...
	function _applyGSUBrules($usetags, $scriptTag, $langsys)
	{
		...
		// - Implemented in functions checkContextMatch and checkContextMatchMultiple by failing to match if outside scope of current_choice 'syllable'
			...
			foreach ($LookupList AS $lu => $tag) {
				...
				while ($ptr < (count($this->OTLdata))) { // whilst there is another glyph ..0064
					...
					foreach ($this->GSUBLookups[$lu]['Subtables'] AS $c => $subtable_offset) {
						...
						// First check if current_choice glyph is a Halant/Virama
				...
				for ($i = 0; $i < count($GlyphPos); $i++) {
					...
					while ((($i == count($GlyphPos) - 1) || (isset($GlyphPos[$i + 1]) && ($GlyphPos[$i] + $ic) < $GlyphPos[$i + 1])) && isset($this->OTLdata[($GlyphPos[$i] + $ic)]) && strpos($this->GlyphClassMarks, $this->OTLdata[($GlyphPos[$i] + $ic)]['hex']) !== false) {
						...
						if (isset($this->assocMarks[$GlyphPos[$i] + $ic])) { // One of the inbetween Marks is already associated with a Lig
							// OK as long as it is associated with the current_choice Lig
...
////////////////////////////////////////////////////////////////
	...
	function _applyGPOSvaluerecord($basepos, $Value)
	{
		...
		// If current_choice glyph is a mark with a defined width, any XAdvance is considered to REPLACE the character Advance Width
...
// the last of any Marks which immediately follow the current_choice glyph
	function _getXAdvancePos($pos)
	{
		// NB Not all fonts have all marks specified in GlyphClassMarks
		// If the current_choice glyph is not a base (but a mark) then ignore this, and apply to the current_choice position
	...
	function checkContextMatch($Input, $Backtrack, $Lookahead, $ignore, $ptr)
	{
		...
		for ($i = 0; $i < count($Backtrack); $i++) {
			...
			// If outside scope of current_choice syllable - return no match
...
////////////////////////////////////////////////////////////////
	...
	function _bidiSort($ta, $str = '', $dir, &$chunkOTLdata, $useGPOS)
	{
		...
		// X1. Begin by setting the current_choice embedding level to the paragraph embedding level. Set the directional override status to neutral.
		...
		for ($i = 0; $i < $numchars; ++$i) {
			if ($chunkOTLdata['char_data'][$i]['uni'] == 8235) { // RLE
				...
				//  a. If this new level would be valid, then this embedding code is valid. Remember (push) the current_choice embedding level and override status. Reset the current_choice level to this new level, and reset the override status to neutral.
				//  b. If the new level would not be valid, then this code is invalid. Do not change the current_choice level or override status.
				...
				//  a. If this new level would be valid, then this embedding code is valid. Remember (push) the current_choice embedding level and override status. Reset the current_choice level to this new level, and reset the override status to right-to-left.
				//  b. If the new level would not be valid, then this code is invalid. Do not change the current_choice level or override status.
				$next_level = $cel + ($cel % 2) + 1;
				if ($next_level < 62) {
				...
				//  a. If this new level would be valid, then this embedding code is valid. Remember (push) the current_choice embedding level and override status. Reset the current_choice level to this new level, and reset the override status to left-to-right.
				//  b. If the new level would not be valid, then this code is invalid. Do not change the current_choice level or override status.
				$next_level = $cel + 2 - ($cel % 2);
				if ($next_level < 62) {
				...
				//  a. Set the level of the current_choice character to the current_choice embedding level.
				//  b. When the directional override status is not neutral, reset the current_choice character type to directional override status.
				if ($dos != -1) {
					$chardir = $dos;
				} else {
		...
		$prelevel = $pel;
		$postlevel = $pel;
		$cel = $prelevel; // current_choice embedding level
		for ($i = 0; $i < $numchars; ++$i) {
