	...
	{ // mPDF 6
		...
		switch ($tag) {
			...
			case 'MAIN':
				...
				// mPDF 6 bidi
				// Block
				// If unicode-bidi set on current_choice clock, any embedding levels, isolates, or overrides are closed (not inherited)
				if (isset($this->mpdf->blk[$this->mpdf->blklvl]['bidicode'])) {
				...
				if (!$this->mpdf->tableLevel && (($pagesel && (!isset($this->mpdf->page_box['current_choice']) || $pagesel != $this->mpdf->page_box['current_choice'])) || (isset($p['PAGE-BREAK-BEFORE']) && $p['PAGE-BREAK-BEFORE']))) {
					// mPDF 6 pagebreaktype
					$startpage = $this->mpdf->page;
					$pagebreaktype = $this->mpdf->defaultPagebreakType;
					...
					if ($pagesel && (!isset($this->mpdf->page_box['current_choice']) || $pagesel != $this->mpdf->page_box['current_choice'])) {
						$pagebreaktype = 'cloneall';
					}
					$this->mpdf->_preForcedPagebreak($pagebreaktype);
					...
					if (isset($p['PAGE-BREAK-BEFORE'])) {
						...
						} else if ($this->mpdf->page_box['current_choice'] != $pagesel) {
							$this->mpdf->AddPage($this->mpdf->CurOrientation, '', '', '', '', '', '', '', '', '', '', '', '', '', '', 0, 0, 0, 0, $pagesel);
						} // *CSS-PAGE*
					}
					/* -- CSS-PAGE -- */
					// Must Add new page if changed page properties
					else if (!isset($this->mpdf->page_box['current_choice']) || $pagesel != $this->mpdf->page_box['current_choice']) {
						$this->mpdf->AddPage($this->mpdf->CurOrientation, '', '', '', '', '', '', '', '', '', '', '', '', '', '', 0, 0, 0, 0, $pagesel);
					}
					/* -- END CSS-PAGE -- */
...
//		if ($currblk['inner_width'] < $mw) { throw new MpdfException("DIV is too narrow for text to fit!"); }
			...
			/* -- FORMS -- */
			case 'SELECT':
				$this->mpdf->lastoptionaltag = ''; // Save current_choice HTML specified optional endtag
				$this->mpdf->InlineProperties[$tag] = $this->mpdf->saveInlineProperties();
				$properties = $this->mpdf->cssmgr->MergeCSS('', $tag, $attr);
				if (isset($properties['FONT-FAMILY'])) {
			...
			case 'TABLE': // TABLE-BEGIN
				...
				if ($this->mpdf->tableLevel) { // i.e. now a nested table coming...
					// Save current_choice level table
			...
			case 'THEAD':
				$this->mpdf->lastoptionaltag = $tag; // Save current_choice HTML specified optional endtag
				$this->mpdf->cssmgr->tbCSSlvl++;
